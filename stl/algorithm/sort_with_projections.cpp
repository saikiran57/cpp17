...
public:
    ...
    MyObject(int p1, double p2, const std::string &p3) 
        : property1(p1), property2(p2), property3(p3) {}
};
...
int main() {
    ...
    sortWithProjection(vec, [](const MyObject &obj) { return obj.property1; });
    std::cout << "Sorted by property1:" << std::endl;
    printVector(vec);

    // Sort by property2 using projection
    sortWithProjection(vec, [](const MyObject &obj) { return obj.property2; });
    std::cout << "Sorted by property2:" << std::endl;
    printVector(vec);

    // Sort by property3 using projection
    sortWithProjection(vec, [](const MyObject &obj) { return obj.property3; });
